...
class Controller_CrudBigTemplate extends \Controller_BaseBigTemplate {
    ...
    protected $_crud_actual_object_identifier = null;
    ...
    {
        if(!substr($this->_crud_actual_object, 0, 7) == '\Model_') {
            throw \Exception(__('exception.srit.controller_crudbigtemplate._extract_crud_object.object_type.not_allowed', array('actual_object' => $this->_crud_actual_object)));
        }
        $this->_crud_objects[$this->_crud_actual_object]['model_object_name'] = $this->_crud_actual_object;
        $this->_crud_objects[$this->_crud_actual_object]['model_object_identifier'] = $this->_crud_actual_object_identifier = strtolower(substr($this->_crud_actual_object, 7));
    }
    ...
    protected function _iterate_crud_objects()
    {
        ...
        foreach ($this->_crud_objects as $key => $crud) {
            ...
            $this->_get_content_partial()
                ->set($this->_crud_objects[$this->_crud_actual_object]['model_object_identifier'], $this->_crud_objects[$this->_crud_actual_object]['data'], false);
    ...
    protected function _paginate()
    {
        ...
        $this->_crud_pagination[$this->_crud_objects[$this->_crud_actual_object]['model_object_identifier']] = \Pagination::forge($this->_crud_actual_object, $this->_crud_pagination_config[$this->_crud_actual_object]);
        $this->_crud_objects[$this->_crud_actual_object]['options']['limit'] = $this->_crud_pagination[$this->_crud_objects[$this->_crud_actual_object]['model_object_identifier']]->per_page;
        $this->_crud_objects[$this->_crud_actual_object]['options']['offset'] = $this->_crud_pagination[$this->_crud_objects[$this->_crud_actual_object]['model_object_identifier']]->offset;
    }
    ...
    protected function _crud_filter()
    {
        ...
        if ($filter_type = \Input::get('filter_type', false) OR $filter_data = \Input::get($this->_crud_actual_object_identifier, false)) {
    ...
    {
        $model_identifier = $this->_crud_actual_object_identifier;
        if ($get = \Input::get()
            AND (isset($get['order']) OR isset($get[$model_identifier]['order']))
                AND (isset($get['order_field']) OR isset($get[$model_identifier]['order_field']))
                    AND (isset($get['order_type']) OR isset($get[$model_identifier]['order_type']))
        ) {
            $order_field = isset($get['order_field']) ? $get['order_field'] : $get[$model_identifier]['order_field'];
            $order_type = isset($get['order_type']) ? $get['order_type'] : $get[$model_identifier]['order_type'];
